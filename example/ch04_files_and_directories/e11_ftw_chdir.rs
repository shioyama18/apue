...
use libc::{
    chdir, closedir, lstat, opendir, readdir, stat, S_IFBLK, S_IFCHR, S_IFDIR, S_IFIFO, S_IFLNK,
...
impl Counter {
    fn ftw(&mut self, pathname: &CStr) -> Result<()> {
        ...
        unsafe {
            ...
            chdir(pathname.as_ptr()).non_negative()?;
            self.ndir += 1;
            let dp = opendir(cwd.as_ptr()).not_null()?;
            while let Ok(dirp) = readdir(dp).not_null() {
                let d_name = CStr::from_ptr((*dirp).d_name.as_ptr());
                if d_name != cwd.as_c_str() && d_name != parent.as_c_str() {
                    lstat(d_name.as_ptr(), &mut statbuf).non_negative()?;
                    match statbuf.st_mode & S_IFMT {
                        S_IFDIR => {
                            self.ftw(d_name)?;
                            chdir(parent.as_ptr()).non_negative()?;
                        }
                        S_IFREG => self.nreg += 1,
                        S_IFBLK => self.nblk += 1,
                        S_IFCHR => self.nchr += 1,
